<form action="" class="property_search results_search">
    <fieldset>
        <div class="container">
            <div class="search__sale_type">
                <div class="btn_group form_group">
                    <label for="" class="active">
                        <span>Buy <span class="desktop">&nbspProperty</span> </span> </label>
                    <label for=""><span>Rent<span class="desktop">&nbspProperty</span></span></label>
                </div>
            </div>
            <div class="search__location_radius form_group">
                <div class="location_radius__location">
                    <input type="text">
                </div>
                <div class="location_radius__radius">
                    <select name="" >
                        <option value="1">1 Mile</option>
                    </select>
                </div>
            </div>
            <div class="search__property_price form_group">
                <div class="property_price__from">
                    <select name="price from" >
                        <option value="100000">£100,000</option>
                        <option value="200000">£200,000</option>
                        <option value="300000">£300,000</option>
                        <option value="400000">£400,000</option>
                    </select>
                </div>
                <div class="property_price__to">
                    <select name="price to" >
                        <option value="100000">£100,000</option>
                        <option value="200000">£200,000</option>
                        <option value="300000">£300,000</option>
                        <option value="400000">£400,000</option>
                    </select>
                </div>
            </div>
            <div class="search__bedrooms form_group">
                <select name="bedrooms" >
                    <option value="1">1+ Bedrooms</option>
                    <option value="2">2+ Bedrooms</option>
                    <option value="3">3+ Bedrooms</option>
                </select>
            </div>
            <div class="search__sale_agreed search__form_group search__field_boolean">
                <input type="checkbox" name="sale agreed" >

                <span>Sale Agreed</span>
            </div>
            <div class="search__new_build form_group field_boolean">
                <input type="checkbox" name="Sale Agreed" >
                <span>New Build only</span>
            </div>
            <div class="search__chain_free form_group field_boolean">
                <input type="checkbox" name="chain free" >
                <span>Chain Free Only</span>
            </div>
            <div class="btn_group form_group search__advanced_btn">
                <a href="#" class="btn">Filter</a>
            </div>
        </div>


    </fieldset>
</form>
